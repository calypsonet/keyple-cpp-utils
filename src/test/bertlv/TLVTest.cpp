...
TEST(TLVTest, parse)
{
    std::shared_ptr<Tag> tag1 =
        std::make_shared<Tag>(0x04, Tag::TagClass::CONTEXT, Tag::TagType::PRIMITIVE, 1);
    std::shared_ptr<Tag> tag2 =
        std::make_shared<Tag>(0x04, Tag::TagClass::CONTEXT, Tag::TagType::CONSTRUCTED, 1);
...
TEST(TLVTest, parse_TagNull)
{
    ...
    EXPECT_THROW(tlv.parse(nullptr, 0), IllegalArgumentException);
}
...
TEST(TLVTest, parse_TooLargeIndex)
{
    ...
    EXPECT_THROW(tlv.parse(tag1, 20), IllegalArgumentException);
}
...
TEST(TLVTest, getPosition)
{
    ...
    /* Two TLV */
    TLV tlv({0x84, 0x05, 0x00, 0x11, 0x22, 0x33, 0x44, 0x84, 0x05, 0x55, 0x66, 0x77, 0x88, 0x99});
