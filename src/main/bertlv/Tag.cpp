...
Tag::Tag(const int tagNumber, const TagClass tagClass, const TagType tagType, const int tagSize)
: mTagNumber(tagNumber), mTagClass(tagClass), mTagType(tagType), mTagSize(tagSize)
{
    if (tagClass > TagClass::PRIVATE) {
        throw IllegalArgumentException("TLV Tag: unknown tag class.");
    }
}

Tag::Tag(const std::vector<uint8_t>& binary, const int offset)
{
    if (offset >= static_cast<int>(binary.size())) {
        throw IllegalArgumentException("offset out of bound");
    }

    /* The 2 first bits (b7b6) of the first byte defines the class */
    mTagClass = static_cast<TagClass>((binary.at(offset) & 0xC0) >> 6);
    ...
    /* */
    int number = binary.at(offset) & 0x1F;
    if (number == 0x1F) {
        /* Two-byte tag */
        number = binary.at(offset + 1);
        mTagSize = 2;
    } else {
        /* One-byte tag */
        mTagSize = 1;
